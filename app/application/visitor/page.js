...
export default function Dashboard(props) {
        ...
        if (data){
          if (onlineUser){
            setSells(data.result.filter(function(item){
              return item.userId == onlineUser.id
            }))
          }
        }
      })
  ...
  const loadPrintableTicketPage = function(event, id){
    event.preventDefault()
    router.replace('/application/visitor/printable/ticket?id='.concat(id))
  }

  const loadInvoicePage = function(event, id){
    event.preventDefault()
    router.replace('/application/visitor/printable/invoice?id='.concat(id))
  }
  ...
  return (
    <Grid container position={"fixed"}  flexDirection="row" justifyContent="flex-start" alignContent={"flex-start"}  alignItems="stretch" sx={{ width:"100%", height:"100vh", backgroundColor: grey[100]}}>
      ...
      <Grid item xs={12} sm={12}>
        <NavigationSystem indicator={"visitor"} element={"Mon Espace"}/>
      </Grid>
      {/** Category Buttons */}
