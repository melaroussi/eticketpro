...
export default function Dashboard(props) {
  ...
  const loadCreatePage = function(event, id){
    event.preventDefault()
    router.replace('/application/cashier/graphics/add-sell')
  }
  ...
  const loadPrintableTicketPage = function(event, id){
    event.preventDefault()
    router.replace('/application/cashier/graphics/printable/ticket?id='.concat(id))
  }

  const loadInvoicePage = function(event, id){
    event.preventDefault()
    router.replace('/application/cashier/graphics/printable/invoice?id='.concat(id))
  }
  ...
  return (
    <Grid container position={"fixed"}  flexDirection="row" justifyContent="flex-start" alignContent={"flex-start"}  alignItems="stretch" sx={{ width:"100%", height:"100vh", backgroundColor: grey[100]}}>
      ...
      <Grid item xs={12} sm={12}>
        <NavigationSystem indicator={"tickets"} element={"Espace Billeterie Graphique"}/>
      </Grid>
      {/** Category Buttons */}
                      ...
                      if (index >= selectedTicketsPage*TABLE_ROWS_PER_PAGE && index <= (selectedTicketsPage+1)*TABLE_ROWS_PER_PAGE-1){
                        return(
                          <TableRow key={ row.id }>
                            ...
                            <TableCell align="right">
                              <Button onClick={(e)=>cancelTicketSell(e, row.id)} disabled={row.canceled} variant="text">
                                Annuler 
                              </Button>
                              <Button onClick={(e)=>loadPrintableTicketPage(e, row.id)} disabled={row.printed} variant="text" endIcon={<PrintIcon/>}>
                                Ticket
                              </Button>
                              <Button onClick={(e)=>loadInvoicePage(e, row.id)} variant="text" endIcon={<PrintIcon/>}>
                                Facturette
                              </Button>
                            </TableCell>
                          </TableRow>
