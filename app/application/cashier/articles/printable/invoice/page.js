...
export default function Dashboard(props) {
  ...
  React.useEffect(function(){
    ...
    fetch(process.env.API_USER_ENDPOINT.concat("/articles/sells?id=").concat(id), OPTIONS).then(function(response){
            ...
            <Document title={"Dacturette eTicket Pro"} pdfVersion="1.4"  >
              <Page size={"A5"} style={styles.page} >
                ...
                <View style={styles.section}>
                  ...
                  <Text style={styles.subheader}>Facturette - Article</Text>
                </View>
                
                <View style={styles.section}>
                  <Text style={styles.title}>Section 1</Text>
                  <Text style={styles.subtitle}>Identification de l'Agent</Text>
                ...
                <View style={styles.section}>
                  ...
                  <Text style={styles.label}>Label : <Text style={styles.paragraph}>{ sellInfos.label }</Text></Text>
                  <Text style={styles.label}>Prix Unitaire (HT) : <Text style={styles.paragraph}>{ parseFloat(sellInfos.price).toFixed(2) } DH</Text></Text>
                  <Text style={styles.label}>Taux de TVA (%) : <Text style={styles.paragraph}>{ parseFloat(sellInfos.VATRate).toFixed(2) } %</Text></Text>
                  <Text style={styles.label}>TVA : <Text style={styles.paragraph}>{ parseFloat(sellInfos.price*sellInfos.VATRate/100).toFixed(2) } DH</Text></Text>
                  <Text style={styles.label}>Prix Unitaire (TTC) : <Text style={styles.paragraph}>{ parseFloat(sellInfos.price*(1+sellInfos.VATRate/100)).toFixed(2) } DH</Text></Text>
