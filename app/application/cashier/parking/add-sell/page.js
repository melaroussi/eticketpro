...
export default function Dashboard(props) {
  ...
  const save = async function(event){
    ...
    if (false){
      ...
      fetch(process.env.API_USER_ENDPOINT.concat("/tickets/sells?operation=sell"), OPTIONS).then(function(response){
        response.json().then(function(data){
          router.replace('/application/cashier/parking')
        })
  ...
  return (
    <Grid container position={"fixed"}  flexDirection="row" justifyContent="flex-start" alignContent={"flex-start"}  alignItems="stretch" sx={{ width:"100%", height:"100vh", backgroundColor: grey[100]}}>
      ...
      { /** Category Buttons */}
      <Grid item xs={12} sm={12} height={"45vh"}>
                  ...
                  return(
                    <Card key={item.id} sx={{ width: 300 }}>
                      ...
                      <CardActions disableSpacing disableGutters>
                        <Button onClick={(e)=>addToCart(e, item)} disabled={isAlreadyAdded(item.id)} variant={"contained"} color={"inherit"} fullWidth>
                          Ajouter
                        </Button>
                      </CardActions>
                    </Card>
      ...
      <Grid item xs={12} sm={12} height={"35vh"}>
        <Container maxWidth="xl">
          <Typography variant="h6">
            Panier
          </Typography>
      ...
      <Grid item xs={12} sm={12} height={"5vh"}>
        <Paper  sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 40, paddingY: 1 }}>
          <Container maxWidth="xl">
            <Stack direction={"row"} spacing={2}>
              <Button onClick={(e)=>handleLinks(e, "cashier/parking")} variant="contained" color="inherit" fullWidth>
                Annuler
              </Button>
