...
export default function Page() {
  ...
  const distroy = function(event){
    ...
    fetch(process.env.API_USER_ENDPOINT.concat("/reports?id=").concat(id), OPTIONS).then(function(response){
      response.json().then(function(data){
      ...
      <Grid item xs={12} sm={12} sx={{paddingTop: 10}}>
        ...
        <Container maxWidth={'xs'}>     
          <Card sx={{ display: 'flex', flexDirection: 'column' }}>
            <CardHeader avatar={<Avatar sx={{ bgcolor: blue[600] }} variant="rounded"><StorageIcon /></Avatar>} title="Détails du PV de Clôture" subheader="Suppression" /> 
              ...
              <Stack spacing={2}>
                ...
                </Typography>
                {
                  reports && <Stack spacing={0}>
                    <Typography variant='button'>Taille : { reports.TPETurnover }</Typography>
                    <Typography variant='button'>Largeur : { reports.cashTurnover }</Typography>
                    <Typography variant='button'>Hauteur : { reports.webTurnover }</Typography>
