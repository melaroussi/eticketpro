...
export default function Dashboard(props) {
  ...
  const load = function(){
    ...
    /** Loading Ticket Infos */
    fetch(process.env.API_USER_ENDPOINT.concat("/tickets/access/scans?id=").concat(id), OPTIONS).then(function(response){
      response.json().then(function(data){
        let list = data.result
        /** List */
        setZoneScans(list)
  ...
  /** Add From Cart */
  const plusOne = function(event, id, scanNumber){
    event.preventDefault()
  ...
  /** Remove From Cart */
  const minusOne = function(event, id, scanNumber){
    event.preventDefault()
    ...
    const OPTIONS = {
      ...
      body: JSON.stringify({
        scanNumber: (scanNumber > 0) ? scanNumber - 1 : scanNumber
      })
    }
                  ...
                  <TableBody>
                    {
                      zoneScans.map(function(row){
                        return(
                          <TableRow key={ row.id }>
                            <TableCell align="left">{ row.zoneLabel }</TableCell>
                            <TableCell align="left">{ row.scanNumber }</TableCell>
                            <TableCell>
                              <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                <Button onClick={(e)=>minusOne(e, row.id, row.scanNumber)}>
                                  <RemoveCircle/>
                                </Button>
                                <Button onClick={(e)=>plusOne(e, row.id, row.scanNumber)}>
                                  <AddCircleIcon/>
                                </Button>
                              </ButtonGroup>
                            </TableCell>
                            <TableCell align="right">
                              <Button color="primary" onClick={(e)=>activateZoneReaders(e, row.zoneId)}>
