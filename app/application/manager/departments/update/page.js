...
export default function Page() {
  ...
  const update = async function(event){
    ...
    if (!label){
      ...
      fetch(process.env.API_USER_ENDPOINT.concat("/departments?operation=update&id=").concat(id), OPTIONS).then(function(response){
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, width: { xs : '100%', sm: `calc(100% - ${DRAWER_WIDTH}px)`} }}>
        ...
        <Container maxWidth={'sm'}>
          <Card sx={{ display: 'flex', flexDirection: 'column' }}>
            <CardHeader avatar={<Avatar sx={{ bgcolor: blue[600] }} variant="rounded"><AddCircleIcon /></Avatar>} title="Gestion des Départements" subheader="Modification" />
