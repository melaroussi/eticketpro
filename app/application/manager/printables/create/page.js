...
export default function Dashboard(props) {
  ...
  const save = async function(event){
    ...
    if (!size || !elementAX || !orientation || !elementAY || !elementBX || !elementBY || !elementCX || !elementCY || !elementDX || !elementDY){
      ...
      fetch(process.env.API_USER_ENDPOINT.concat("/printables?operation=create"), OPTIONS).then(function(response){
        router.replace('/application/manager/printables')
      })
    }
  }
            ...
            <CardContent spacing={2}>
              ...
              <Collapse in={validationAlert}>
                ...
                <Typography variant="body1" color="text.secondary">
                  Nous vous prions de saisir les champs obligatoires portants la mention (*). Les autres champs sont optionnels mais il est recommandé de saisir l'ensemble des données. Merci de saisir les dimensions et coordonnées des elements en Pixels sachant que l'affichage est en 76DPI (1mm = {MM_PIXELS_RATIO } Pixels).
                </Typography>
                ...
                <Stack spacing={2}>
                  ...
                  <TextField value={orientation} onChange={handleOrientation} variant='outlined' size='small' label="Orientation" required fullWidth InputLabelProps={{ shrink: true }} select>
                    <MenuItem key={"portrait"} value={"portrait"} >Portrait</MenuItem>
                    <MenuItem key={"landscape"} value={"landscape"} >Paysage </MenuItem>
                  </TextField>
