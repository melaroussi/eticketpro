...
export default function Dashboard(props) {
  ...
  const loadSupplyPage = function(event, id){
    ...
    router.push("/application/manager/supplies/create?id=".concat(id))
  }

  const loadReadPage = function(event, id){
    event.preventDefault()
    router.push("/application/manager/supplies/read?id=".concat(id))
  }
                    ...
                    articles.map(function(row){
                      return(
                        <TableRow key={ row.id }>
                          ...
                          <TableCell align="left">{ row.availableQuantity === 0 ? <SentimentVeryDissatisfiedIcon sx={{color: red[500]}}/>: <SentimentSatisfiedAltIcon sx={{color: green[500]}}/> }</TableCell>
                          <TableCell align="right">
                            <Button variant="text" onClick={(e)=>loadSupplyPage(e, row.id)}>
                              Approvisionnement
                            </Button>
                          </TableCell>
                        </TableRow>
