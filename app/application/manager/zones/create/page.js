...
export default function Dashboard(props) {
  ...
  const save = async function(event){
    ...
    if (!label){
      ...
      fetch(process.env.API_USER_ENDPOINT.concat("/zones?operation=create"), OPTIONS).then(function(response){
        router.replace('/application/manager/zones')
      })
    }
  }
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, width: { xs : '100%', sm: `calc(100% - ${DRAWER_WIDTH}px)`} }}>
        ...
        <Container maxWidth={'sm'}>
          ...
          <Card sx={{ display: 'flex', flexDirection: 'column' }}>
            <CardHeader avatar={<Avatar sx={{ bgcolor: blue[600] }} variant="rounded"><AddCircleIcon /></Avatar>} title="Gestion des Zones" subheader="Enregistrement" />
